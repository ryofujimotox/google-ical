...
def main() -> int:
    ...
    def _run() -> None:
        ...
        output_path = str(config.events_json_dir / gomi_config.output_file)
        ...
        events = convert_gomi_pdf(config, pdf_bytes)
        log_stage_success("PDF→JSON 変換", detail=f"events={len(events)}")
